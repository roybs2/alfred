...
Keep the current PTY integration as the native interactive-session path. Add a separate structured subprocess adapter path for room collaboration. Use the installed provider CLIs, their existing sign-in and permissions, and a per-process Alfred MCP bridge exposing only `room_send` and `room_spawn`. The existing parent conductor calls these tools directly with an explicit target, the task text, the exact context the user/agent authorized to send, and provenance. Do not add a router model, rewrite context, or imply that Claude/Codex redirects its private native subagents into other providers.
